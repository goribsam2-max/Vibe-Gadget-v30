...
export default function AccountMenu({
  ...
  const triggerClass = isPill
    ? cn(
        "flex items-center justify-center p-0 md:pl-2 md:pr-4 gap-2 rounded-full font-medium transition-all duration-300 w-9 h-9 md:w-auto md:h-9 shrink-0 bg-transparent border-none text-zinc-700 dark:text-zinc-200 hover:bg-zinc-200 dark:hover:bg-zinc-700 shadow-none",
      )
    : cn(
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className={triggerClass}>
          {user ? (
            ...
            <div className="w-8 h-8 md:w-7 md:h-7 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center shrink-0">
      ...
      <DropdownMenuContent
        align="end"
        className="w-56 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-lg p-2 z-[9999]"
      >
        {!user ? (
          <>
